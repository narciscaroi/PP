...
(struct bird-structure(x y v-y))

;(define initial-bird (make-bird bird-x bird-initial-y 0))

(struct state-structure(bird-structure pipes score variables abilities))
...
(struct pipe-structure (gap-y x))

;(define (make-pipe x)
 ; (pipe-structure (+ added-number (random random-threshold)) x))
  
;(define initial-pipe (make-pipe scene-width))

;(define pipes (cons initial-pipe '()))
...
(define (get-initial-state)
  (state-structure (bird-structure bird-x bird-initial-y 0)
                   (list (pipe-structure (+ added-number (random random-threshold)) scene-width))
                   0 0 0))
...
(define (get-bird state)
  (state-structure-bird-structure state))

(define (get-bird-y bird)
  (bird-structure-y bird))
;TODO 3
...
(define (next-state-bird bird gravity)
 (struct-copy bird-structure bird
              [y (+ (bird-structure-y bird) (bird-structure-v-y bird))]
              [v-y (+ (bird-structure-v-y bird) gravity)]))

;TODO 4
; După aceasta, implementati un getter care extrage din structura voastră
; viteza pe y a păsării.
(define (get-bird-v-y bird)
  (bird-structure-v-y bird))
...
(define (next-state-bird-onspace bird momentum)
  (struct-copy bird-structure bird [v-y (- 0 momentum)]))
...
(define (change current-state pressed-key)
   (if (key=? pressed-key " ")
       (struct-copy state-structure current-state
                    [bird-structure (next-state-bird-onspace (get-bird current-state)
                                                             initial-momentum)])
      current-state))
...
(define (move-pipes pipes scroll-speed)
  (map (λ (ppipe) (struct-copy pipe-structure ppipe [x (- (pipe-structure-x ppipe) scroll-speed)])) pipes))
;TODO 12
...
(define (clean-pipes pipes)
 (filter (λ(ppipe) (>= (+ (pipe-structure-x ppipe) pipe-width) 0)) pipes))
...
(define (add-more-pipes pipes)
 (if (< (length pipes) no-pipes)
      (append pipes
              (list (pipe-structure (+ added-number (random random-threshold))
                                    (+ pipe-width pipe-gap (pipe-structure-x (last pipes))))))
      pipes)
  )
...
(define (check-ground-collision bird)
 (if (>= (+ (bird-structure-y bird) bird-height) ground-y)
     #t
     #f))
...
(define (invalid-state? state)
  (or (check-ground-collision (state-structure-bird-structure state))
      (check-pipe-collisions (state-structure-bird-structure state) (state-structure-pipes state))))
...
;verific coliziunea cu pipe-ul de sus

(define (check-col-pipe-up bbird ppipe)
  (check-collision-rectangles (make-posn (bird-structure-x bbird) (bird-structure-y bbird) ) ;B1
                              (make-posn (+ (bird-structure-x bbird) bird-width) (+ (bird-structure-y bbird) bird-height) ) ;B2
                              (make-posn (pipe-structure-x ppipe) 0 ) ;A1
                              (make-posn (+ (pipe-structure-x ppipe) pipe-width) (pipe-structure-gap-y ppipe))  ;A2
                              )
  )

;cu pipe-ul de jos
(define (check-col-pipe-down bbird ppipe)
  (check-collision-rectangles (make-posn (bird-structure-x bbird) (bird-structure-y bbird) ) ;B1
                              (make-posn (+ (bird-structure-x bbird) bird-width) (+ (bird-structure-y bbird) bird-height) ) ;B2
                              (make-posn (pipe-structure-x ppipe) (+ (pipe-structure-gap-y ppipe) pipe-self-gap)) ;A'1
                              (make-posn (+ (pipe-structure-x ppipe) pipe-width) (+ scene-width ground-height)) ;A'2
                              )
  )

;preiau pipe-ul si apelez verificarile pe bucati
(define (check-pipe-collisions-helper bird ppipe)
  (if (or (check-col-pipe-up bird ppipe)
          (check-col-pipe-down bird ppipe))
      #t
      #f))

;trimit cate un pipe
(define (check-pipe-collisions bird pipes)
  (displayln (bird-structure-v-y bird))
  (if (null? pipes)
      #f
      (if (check-pipe-collisions-helper bird (car pipes))
          #t
...
(define (next-state state)
  (struct-copy state-structure state
               [bird-structure (next-state-bird (state-structure-bird-structure state) initial-gravity)]
               [pipes (next-state-pipes (state-structure-pipes state) initial-scroll-speed)]
               [score (+ (state-structure-score state) 0.1)]
              ))
...
(define (draw-frame state)
  (place-image bird-image
               ...
               (place-image ground-image
                            ...
                            (place-image (score-to-image (state-structure-score state))
                                         text-x
                                         text-y
                                         (place-pipes (get-pipes state) initial-scene))))
)
; Folosind `place-image/place-images` va poziționa pipe-urile pe scenă.
(define (create-pipes-center-coords pipes coords)
  (if (null? pipes)
      (reverse coords)
      (create-pipes-center-coords (cdr pipes)
                                  (cons (make-posn (+ (pipe-structure-x (car pipes)) (quotient pipe-width 2))
                                                      ;(+ (pipe-structure-gap-y (car pipes))
                                                            (+ pipe-self-gap
                                                               (pipe-structure-gap-y (car pipes))
                                                               (quotient (- pipe-height
                                                                            (+ pipe-self-gap
                                                                               (pipe-structure-gap-y (car pipes)) ))
                                                                         2)))
                                              (cons (make-posn (+ (pipe-structure-x (car pipes)) (quotient pipe-width 2))
                                                         (quotient (pipe-structure-gap-y (car pipes)) 2))
                                                    coords)))))
  
(define (pipes-images pipes images)
 (if (null? pipes)
      (reverse images) ;intorc deoarece este inversata
      (pipes-images (cdr pipes) (cons (rectangle pipe-width (- pipe-height (+ pipe-self-gap (pipe-structure-gap-y (car pipes))))  "solid" "green") ;pipe de jos
                                            (cons (rectangle pipe-width (pipe-structure-gap-y (car pipes))  "solid" "green") ;pipe de sus
                                            images)))))

(define (place-pipes pipes scene)
  (place-images (pipes-images pipes '()) (create-pipes-center-coords pipes '()) scene))
; Bonus
